...
| TD-14 | O parser Agro (`src/closer_ai/ingestion/agro/parser.py`) tem resolução explícita de `role="closer"` (`_resolve_closer`), mas nenhuma resolução equivalente para `role="lead"` — participantes não-closer ficam `role="unknown"` a menos que a fonte bruta já traga `role="lead"` explícito (hipótese não confirmada). Achado por auditoria independente (AI Engineer + Domain Agent) durante o onboarding de dado real. | `RuleBasedObjectionExtractor` só escaneia segmentos `role="lead"` — sem essa resolução, qualquer call real processada produz `Call` válido com **zero objeções detectadas**, silenciosamente, sem erro ou flag. Maior risco de silent-failure identificado até agora. | P0 | Antes de qualquer avaliação do extractor v0 contra dado real — bloqueador do gate 5-CALL VALIDATION PASSED (`docs/context/EXECUTION_GATES.md`), verificado explicitamente no passo 13 de `docs/data/FIRST_CALLS_VALIDATION.md`. | Acesso a 5 calls reais para confirmar se a fonte de fato marca `role="lead"`, ou se é preciso uma regra de resolução nova (ex.: "não-closer com fala relevante = lead") | Não nesta task — é mudança de parser, fora do escopo desta rodada de documentação; mas é o item de maior prioridade real para a próxima branch de engenharia sobre Agro. |
