...
| TD-14 | **MITIGATED / REAL DATA VALIDATION REQUIRED.** O parser Agro (`src/closer_ai/ingestion/agro/parser.py`) resolve explicitamente `role="closer"` (`_resolve_closer`), mas não infere `role="lead"` quando a fonte não fornece evidência explícita. Participantes não-closer permanecem `role="unknown"`. A mitigação adiciona a quality flag `unresolved_lead` quando existe transcript utilizável, mas nenhum lead semanticamente resolvido. Inferência por topologia (`closer + 1 outro = lead`) foi avaliada e rejeitada por revisão adversarial por produzir falsos positivos plausíveis em shadow calls, SDR handoffs, treinamento e outros cenários. | Antes da mitigação, `RuleBasedObjectionExtractor` podia retornar zero objeções silenciosamente quando nenhum participante resolvia para `role="lead"`. Agora esse caso continua conservador (`[]`), mas é diagnosticável via `Call.quality_flags`. Testes unitários, stress e integração Agro → Call → ObjectionExtractor protegem esse comportamento. | P1 | Validar contra as primeiras calls Agro reais. Se a fonte trouxer identidade/role confiável, usar essa evidência; caso contrário, definir resolução apoiada em fonte confiável/CRM sem inferir identidade por exclusão. | Acesso e autorização para amostra real Agro/Zoom. | Mitigação concluída; não adicionar nova inferência antes da validação com dados reais. |
